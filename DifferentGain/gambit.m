...
%             end
		...
		% 	
                % players è composto come segue: [intruso, difensore chiamante, altri difensori] 
                % defensor è composto come segue: [difensore chiamante,altri difensori]
...
%                         end
                        if p==1
                            % funzione costo ipotizzata dai difensori per
                            % l'intruso
                            I(p)= - l_dist_player_critic-defensors(p).obstacle_factor*(1/l_min_dist_player_nearobs_otherplayer);
                        else
                            I(p)= - defensors(1).obstacle_factor*(1/l_min_dist_player_nearobs_otherplayer) -defensors(1).target_factor*l_dist_player_arc -defensors(1).barrier_factor*barrier_gain;
                        end
...
%                    if world.time <= 500
%                        obj.EqCount(world.time)=numEquilibriums;
%                        world.time
%                        temp=obj.EqCount;
%                        save('EqCount.mat','temp');
%                    end
                   EqLen=numPlayers*3;
