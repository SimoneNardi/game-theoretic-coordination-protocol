    ...
    methods
        ...
        function draw(obj)
            ...
            if obj.draw_enviroment==0
                ...
                fig=figure('Position',[1 1 1000 1000],'keypressfcn',@intruder_handler.key_pressed);
                ...
                for d=1:size(obj.critAreas,1)
                    ...
                    text(obj.critAreas(d,1),obj.critAreas(d,2),'C','Color','black');
                    if obj.colors(d)== 'b'
                    text(obj.critAreas(d,1),obj.critAreas(d,2),'C','Color','white');
                    end
                end 
       ...
       %% condizioni di uscita della simulazione
        ...
        function [t, exitStatus]=start(obj)
            ...
            while 1
                ...
                obj.intruder.move();               
                IntrPos(obj.time,:)=obj.intruder.currentPosition;
                save('IntrPos.mat','IntrPos');
                for i=1:length(obj.defenders)
                    obj.defenders(i).move();                    
                    if i==1                        
                    RobotPos1(obj.time,:)=obj.defenders(i).currentPosition;
                    save('RobotPos1.mat','RobotPos1');
                    elseif i==2
                    RobotPos2(obj.time,:)=obj.defenders(i).currentPosition;
                    save('RobotPos2.mat','RobotPos2');
                    else 
                    RobotPos3(obj.time,:)=obj.defenders(i).currentPosition;
                    save('RobotPos3.mat','RobotPos3');        
                    end
                end
